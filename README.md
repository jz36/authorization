Всем привет. В предыдущей статье я описал процесс аутентификации в `Micronaut`. Но аутентификация без авторизации мертва! Ну или крайне мало применима. Поэтому предлагаю заглянуть под кат и оценить различные механизмы авторизации в данном фреймворке, а заодно узнать, как это все тестировать.
...
- Авторизация - ~~процесс установления прав доступа и разрешений для пользователя или системы с целью определения, какие ресурсы и функционал они могут использовать в информационной системе.~~ процесс проверки пользователя на выполнение тех или иных действий.

Скелет приложения с аутентификацией я собираюсь использовать из [предыдущей статьи](https://habr.com/ru/companies/X5Tech/articles/715634/), сделав, однако, апгрейд до четвертой версии `Micronaut`, которая вышла в [июле](https://habr.com/ru/companies/X5Tech/articles/750396/) этого года.
...
Для начала стоит немного рассказать про структуру базы данных. Всего будет 4 таблицы: `user`, `roles`, `rules` и `roles_rules`. Ниже вы найдете их описание.

![database](./static/database.png)

Можно сказать, что это джентльменский набор при реализации авторизации. В целом, все достаточно прозаично, поэтому не будем надолго здесь останавливаться, лишь обозначим некоторые условности:

1. Придерживаемся политики: все, что не разрешено - запрещено.
...
Правила безопасности возвращают `publicher`, который содержит единственный статус `SecurityRuleResult`. Ниже представлена таблица с описанием этих результатов:
...
| [SecuredAnnotationRule](https://micronaut-projects.github.io/micronaut-security/latest/guide/#secured)                 | -200            | Хотя бы одна из необходимых указанных ролей есть у пользователя | У пользователя нет ни одной указанной роли, указанной в аннотации    | Нет аннотаций безопасности у данного ednpoint                                      |
| [ConfigurationInterceptMapRule](https://micronaut-projects.github.io/micronaut-security/latest/guide/#interceptUrlMap) | -100            | Хотя бы одна из необходимых указанных ролей есть у пользователя | У пользователя нет ни одной указанной роли, указанной в конфигурации | Ни один URL-паттерн не совпадает с запросом                                        |
...
С помощью кода, приведенного выше, можно запретить доступ к серверу со всех адресов, которые не перечислены. При этом, такое правило разрешает никаких действий, а только отклоняет все запросы, которые пришли с других IP-адресов.
...
```
@Controller("/example")
@Secured(SecurityRule.IS_AUTHENTICATED) // Доступ к этом контроллеру по умолчанию есть только у аутентифицированных пользователей
...
Так же можно использовать аннотации `Jakarta`:
...
10. `SecurityFilter` - если предыдущий шаг завершился успешно, то данный фильтр сообщит примет решение об авторизации данного запроса

## Как это тестировать

Итак, встает вопрос, как же протестировать, что только пользователь сможет воспользоваться нашим API только при соблюдении определенных условий? К счастью, `Micronaut` предоставляет весь необходимый функционал, и сам процесс достаточно прост.
...
2. Инжектим генератор JWT токенов. Важно отметить, что для теста совсем неважно, какая фраза используется для генерации токенов.
3. Так же потребуется подключить клиент, для того чтобы симулировать как можно более точно сам процесс запроса от пользователя.
...
Другие примеры можно тестирования можно найти в [репозитории](https://github.com/jz36/authorization-micronaut). 
...
На этом разбор работы механизма авторизации в `Micronaut` можно считать завершенным. Надеюсь было интересно и познавательно. На всякий случай оставлю [здесь](https://habr.com/ru/companies/X5Tech/articles/715634/) ссылку на статью об аутентификации.